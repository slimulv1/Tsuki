...
# netpanel: Generate a Wi-Fi share QR matrix for the netpanel C renderer.
# Adapted from omarchy bin/omarchy-network-qr (keeps every design decision:
# secret via nmcli --show-secrets instead of argv, enterprise guard, WIFI:
# payload escaping, WEP handling, ASCII matrix collapse, quiet-zone margin).
# Output contract for netpanel.c cb_qr_done():
#   line 1: meta<TAB>iface<TAB>security<TAB>ssid   (SSID last: may contain tabs)
#   lines 2..N: square rows of 0/1 characters
...
if [[ -z $interface ]]; then
  # Prefer the default-route device; fall back to the first connected Wi-Fi
  # device. Pin the locale (nmcli localizes state names); the prefix match
...
# Margin 4 is the spec quiet zone; the panel draws its own white surround,
# so this border is all the scanner gets.
